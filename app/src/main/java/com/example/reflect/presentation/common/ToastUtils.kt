...
object ToastUtils {
    ...
    fun showSuccessUpdateProfile(context: Context) {
        val view = LayoutInflater.from(context).inflate(R.layout.toast_successful_update, null)
        val toast = Toast(context).apply {
            setView(view)
            setGravity(Gravity.BOTTOM, 0,YOFFSET)
            duration = Toast.LENGTH_SHORT
    ...
    fun showSuccessDeleteUser(context: Context) {
        val view = LayoutInflater.from(context).inflate(R.layout.toast_delete_user, null)
        val toast = Toast(context).apply {
            setView(view)
            setGravity(Gravity.BOTTOM, 0,YOFFSET)
            duration = Toast.LENGTH_SHORT
